...
# returns the mean of numeric vector x
getMean <- function(x) {
    sum(x) / length(x)
}

# returns TRUE if the length of numeric vector x 
# is an even number; FALSE otherwise
isLengthAnEvenNumber <- function(x) {
    length(x) %% 2 == 0
}

# returns the median of numeric vector x
getMedian <- function(x) {
    centerIndices <- ceiling(length(x) / 2)
    if (isLengthAnEvenNumber(x)) {
...
getMean <- function(x) {
    ...
    if (is.character(x)) {
        stop("x must be a numeric vector, but is a character vector.")
    }
    tryCatch({
        sum(x) / length(x)      
    }, error = function(e) {
...
This is an opportunity to anticipate your user's problems. You may prioritize
the biggest errors with custom messages. (**Remember**: `checkmate` can also be
useful in drafting error messages).
...
-   Can be run automatically as part of a CI/CD pipeline
-   Can also be run automatically before committing code changes
...
## Possible workflow - write the unit tests first

**First**, write a placeholder function:
...
**Then**, write your unit tests, which should fail:

```{r, echo = TRUE, eval = FALSE}
library(testthat)
expect_error(getMean(x = c("a","b","c")))
```

::: red-small
Error: `getMean(c("a", "b", "c"))` did not throw the expected error.
:::

**Finally**, go back and edit the function so that the unit tests will pass:
...
How to link the [styler](https://styler.r-lib.org)[^1] package to a keyboard
shortcut:
...
## Please install the following packages before we move on
...
-   Creators (initial authors): Friedrich Pahlke
    [`r fontawesome::fa("github")`](https://github.com/fpahlke/)
    [`r fontawesome::fa("linkedin")`](https://www.linkedin.com/in/pahlke/), Joe
    Zhu [`r fontawesome::fa("github")`](https://github.com/shajoezhu)
    [`r fontawesome::fa("linkedin")`](http://www.linkedin.com/in/joe-zhu-464b5818)
-   In the current version, changes were done by (later authors): Matt Secrest
    [`r fontawesome::fa("github")`](https://github.com/mattsecrest/)
-   This work is licensed under the [Creative Commons Attribution-ShareAlike 4.0
    International License](http://creativecommons.org/licenses/by-sa/4.0/).
-   The source files are hosted at
    [github.com/openpharma/workshop-r-swe-sf](https://github.com/openpharma/workshop-r-swe-sf),
    which is forked from the original version at
    [github.com/openpharma/workshop-r-swe](https://github.com/openpharma/workshop-r-swe/).
-   Important: To use this work you must provide the name of the creators
    (initial authors), a link to the material, a link to the license, and
