...
- Run CRAN like checks in RStudio: use `devtools::check()`
- Look at the CI/CD checks reported on your GitHub repository
  - Where are the `R CMD check` results?
- Enable github page through `usethis::use_pkgdown_github_pages()`
- Deploy the website with `pkgdown`
  - Need to change remote to `git`
- Deploy the website through `usethis::use_github_action("pkgdown")`
...
- In the current version, changes were done by (later authors): Daniel Sjoberg [`r fontawesome::fa("link")`](https://www.danieldsjoberg.com) [`r fontawesome::fa("linkedin")`](https://www.linkedin.com/in/ddsjoberg/) [`r fontawesome::fa("github")`](https://github.com/ddsjoberg/) [`r fontawesome::fa("twitter")`](https://twitter.com/statistishdan)
